...
struct SongInfo {
    ...
    SongInfo() : title(""), artist(""), path("") {}
    SongInfo(const std::string& t, const std::string& a, const std::string& p = "") : title(t), artist(a), path(p) {}
};
...
public:
    ...
    virtual std::vector<SongInfo> GetAvailableSongs() const override;
    virtual bool PlaySongByPath(const std::string& file_path, const std::string& song_name) override;
};
