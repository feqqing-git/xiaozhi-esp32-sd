...
namespace {
constexpr int kSpectrumSampleCount = 256;
constexpr int kSpectrumMinHeight = 8;
constexpr int kSpectrumMaxHeight = 60;

void ComputeSpectrumBars(const int16_t* pcm_data, size_t sample_count, std::array<int, 16>& levels) {
    levels.fill(kSpectrumMinHeight);
    ...
    size_t count = std::min<size_t>(sample_count, fft_size);
    for (size_t i = 0; i < count; ++i) {
        real[i] = static_cast<float>(pcm_data[i]) / 32768.0f;
    }
    ...
    for (size_t len = 2; len <= fft_size; len <<= 1) {
        ...
        const float cos_len = std::cos(angle);
        const float sin_len = std::sin(angle);
        for (size_t i = 0; i < fft_size; i += len) {
            float w_real = 1.0f;
            float w_imag = 0.0f;
            const size_t half_len = len / 2;
            for (size_t j = 0; j < half_len; ++j) {
                const float u_real = real[i + j];
                const float u_imag = imag[i + j];
                const float v_real = real[i + j + half_len] * w_real - imag[i + j + half_len] * w_imag;
                const float v_imag = real[i + j + half_len] * w_imag + imag[i + j + half_len] * w_real;
                real[i + j] = u_real + v_real;
                imag[i + j] = u_imag + v_imag;
                real[i + j + half_len] = u_real - v_real;
                imag[i + j + half_len] = u_imag - v_imag;

                const float next_w_real = w_real * cos_len - w_imag * sin_len;
                const float next_w_imag = w_real * sin_len + w_imag * cos_len;
                w_real = next_w_real;
                w_imag = next_w_imag;
            }
        }
    ...
    const size_t bins = fft_size / 2;
    for (size_t band = 0; band < levels.size(); ++band) {
        const size_t start_bin = (band * bins) / levels.size();
        const size_t end_bin = ((band + 1) * bins) / levels.size();
        float energy = 0.0f;
        for (size_t bin = start_bin; bin < end_bin && bin < bins; ++bin) {
            const float magnitude = std::sqrt(real[bin] * real[bin] + imag[bin] * imag[bin]) / static_cast<float>(fft_size);
            energy += magnitude;
        }
        const float avg_energy = (end_bin > start_bin) ? (energy / static_cast<float>(end_bin - start_bin)) : 0.0f;
        const int height = static_cast<int>(std::clamp(avg_energy * 220.0f, 8.0f, static_cast<float>(kSpectrumMaxHeight)));
        levels[band] = height;
    }
}
}  // namespace
...
LcdDisplay::LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height)
    ...
    esp_timer_create_args_t background_timer_args = {
        .callback = [](void* arg) {
            LcdDisplay* display = static_cast<LcdDisplay*>(arg);
            display->UpdateBackgroundCarousel();
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "background_carousel",
        .skip_unhandled_events = false,
    };
    esp_timer_create(&background_timer_args, &background_carousel_timer_);
}

void LcdDisplay::SetBackgroundCarouselImage(int index) {
    ...
    background_index_ = index % BACKGROUND_COUNT;
    background_image_cached_ = std::make_unique<LvglSourceImage>(get_background_image(static_cast<background_type_t>(background_index_)));

    DisplayLockGuard lock(this);
    lv_obj_set_style_bg_image_src(container_, background_image_cached_->image_dsc(), 0);
}

void LcdDisplay::UpdateBackgroundCarousel() {
    if (container_ == nullptr) {
        return;
    }
    int next_index = (background_index_ + 1) % BACKGROUND_COUNT;
    SetBackgroundCarouselImage(next_index);
}
...
LcdDisplay::~LcdDisplay() {
    ...
    if (background_carousel_timer_ != nullptr) {
        esp_timer_stop(background_carousel_timer_);
        esp_timer_delete(background_carousel_timer_);
    }
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_text_color(status_bar_, lv_color_hex(0x00FF00), 0);
    ...
    lv_obj_set_style_text_color(network_label_, lv_color_hex(0x00FF00), 0);
    ...
    lv_obj_set_style_text_color(mute_label_, lv_color_hex(0x00FF00), 0);
    ...
    SetBackgroundCarouselImage(0);
    if (background_carousel_timer_ != nullptr) {
        esp_timer_start_periodic(background_carousel_timer_, 5000 * 1000);
    }
}
...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    ...
    lv_obj_set_style_bg_opa(img_bubble, LV_OPA_TRANSP, 0);
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Set background image from the carousel if enabled, otherwise use theme background
    if (background_image_cached_ != nullptr) {
        lv_obj_set_style_bg_image_src(container_, background_image_cached_->image_dsc(), 0);
    } else if (lvgl_theme->background_image() != nullptr) {
...
void LcdDisplay::SetMusicInfo(const char* song_name) {
   ...
   if (song_name != nullptr && strlen(song_name) > 0) {
        std::string formatted = "🎵 " + std::string(song_name);
        // 直接调用 SetChatMessage 插入气泡
        SetChatMessage("assistant", formatted.c_str());
    }
    #else
    // 非微信模式：在表情下方显示歌名
    DisplayLockGuard lock(this);
...
void LcdDisplay::SetupMusicSpectrumBars() {
    if (music_panel_ == nullptr) {
        return;
    }

    for (auto& bar : music_spectrum_bars_) {
        if (bar != nullptr) {
            lv_obj_del(bar);
        }
    }
    music_spectrum_bars_.fill(nullptr);

    const int bar_width = 10;
    const int bar_gap = 3;
    const int bar_count = static_cast<int>(music_spectrum_bars_.size());
    int container_width = bar_count * bar_width + (bar_count - 1) * bar_gap;
    int start_x = -container_width / 2;

    for (int i = 0; i < bar_count; ++i) {
        lv_obj_t* bar = lv_obj_create(music_panel_);
        lv_obj_set_size(bar, bar_width, kSpectrumMinHeight);
        lv_obj_align(bar, LV_ALIGN_BOTTOM_MID, start_x + i * (bar_width + bar_gap), -18);
        lv_obj_set_style_bg_color(bar, lv_color_hex(0x8be3ff), 0);
        lv_obj_set_style_bg_opa(bar, LV_OPA_COVER, 0);
        lv_obj_set_style_border_width(bar, 0, 0);
        lv_obj_set_style_radius(bar, 2, 0);
        music_spectrum_bars_[i] = bar;
    }

    UpdateMusicSpectrumBars();
}

void LcdDisplay::UpdateMusicSpectrumBars() {
    if (music_panel_ == nullptr) {
        return;
    }

    if (music_spectrum_bars_[0] == nullptr) {
        SetupMusicSpectrumBars();
        return;
    }

    std::array<int, 16> target_levels{};
    auto* music = Board::GetInstance().GetMusic();
    if (music != nullptr) {
        auto* audio_data = music->GetAudioData();
        if (audio_data != nullptr) {
            ComputeSpectrumBars(audio_data, kSpectrumSampleCount, target_levels);
        }
    }

    for (size_t i = 0; i < music_spectrum_bars_.size(); ++i) {
        lv_obj_t* bar = music_spectrum_bars_[i];
        if (bar == nullptr) {
            continue;
        }

        if (target_levels[i] > 0) {
            music_spectrum_levels_[i] = (music_spectrum_levels_[i] * 2 + target_levels[i]) / 3;
        } else {
            music_spectrum_levels_[i] = std::max(kSpectrumMinHeight, music_spectrum_levels_[i] - 2);
        }

        const int height = std::clamp(music_spectrum_levels_[i], kSpectrumMinHeight, kSpectrumMaxHeight);
        lv_obj_set_height(bar, height);
        if (height <= 20) {
            lv_obj_set_style_bg_color(bar, lv_color_hex(0x8be3ff), 0);
        } else if (height <= 40) {
            lv_obj_set_style_bg_color(bar, lv_color_hex(0xffc14b), 0);
        } else {
            lv_obj_set_style_bg_color(bar, lv_color_hex(0xff7fa2), 0);
        }
        lv_obj_set_style_bg_opa(bar, LV_OPA_COVER, 0);
        lv_obj_set_style_radius(bar, 2, 0);
    }
...
void LcdDisplay::MusicSpectrumTimerCb(lv_timer_t* timer) {
    auto* display = static_cast<LcdDisplay*>(lv_timer_get_user_data(timer));
    if (display == nullptr) {
        return;
    }

    display->music_spectrum_phase_ = (display->music_spectrum_phase_ + 1) % 16;
    display->UpdateMusicSpectrumBars();
}
...
void LcdDisplay::start() {
    ...
    if (music_panel_ != nullptr && !music_panel_visible_) {
        lv_obj_clear_flag(music_panel_, LV_OBJ_FLAG_HIDDEN);
        music_panel_visible_ = true;
    }
    if (music_spectrum_timer_ == nullptr) {
        music_spectrum_timer_ = lv_timer_create(MusicSpectrumTimerCb, 80, this);
        lv_timer_set_user_data(music_spectrum_timer_, this);
    }
    UpdateMusicSpectrumBars();
}
...
void LcdDisplay::clearScreen() {
    if (music_spectrum_timer_ != nullptr) {
        lv_timer_del(music_spectrum_timer_);
        music_spectrum_timer_ = nullptr;
    }
}
...
#if CONFIG_USE_WECHAT_MESSAGE_STYLE
    lv_obj_set_size(music_panel_, width_ - 20, 96);
    lv_obj_align(music_panel_, LV_ALIGN_BOTTOM_MID, 0, -8);
#else
...
void LcdDisplay::SetMusicProgress(const char* song_name, int current_seconds, int total_seconds, float progress_percent) {
    ...
    if (!music_panel_visible_) {
        ...
        if (music_spectrum_timer_ == nullptr) {
            music_spectrum_timer_ = lv_timer_create(MusicSpectrumTimerCb, 120, this);
            lv_timer_set_user_data(music_spectrum_timer_, this);
        }
    }
