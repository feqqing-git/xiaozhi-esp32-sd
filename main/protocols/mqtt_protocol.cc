...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    auto normalize_endpoint = [](std::string value) {
        ...
        // Remove common MQTT URL schemes
        const char* schemes[] = {"mqtt://", "mqtts://", "tcp://", "ssl://"};
        for (const auto& scheme : schemes) {
            size_t scheme_len = strlen(scheme);
            if (value.size() >= scheme_len && value.compare(0, scheme_len, scheme) == 0) {
                value.erase(0, scheme_len);
                break;
            }
        }

        // Remove stray leading colon(s)
    ...
    if (endpoint.empty()) {
        ESP_LOGW(TAG, "MQTT endpoint is invalid after normalization");
        if (report_error) {
            SetError(Lang::Strings::SERVER_NOT_FOUND);
        }
        return false;
    ...
    if (pos != std::string::npos) {
        std::string port_part = endpoint.substr(pos + 1);
        if (!port_part.empty() && std::all_of(port_part.begin(), port_part.end(), [](char c) { return std::isdigit(static_cast<unsigned char>(c)); })) {
            broker_address = endpoint.substr(0, pos);
            broker_port = std::stoi(port_part);
        }
    }
    ESP_LOGI(TAG, "MQTT connect parameters: broker=%s port=%d client_id=%s username=%s publish_topic=%s",
        broker_address.c_str(), broker_port, client_id.c_str(), username.c_str(), publish_topic_.c_str());
